...
def KeccakF1600(state):
    state = np.copy(np.frombuffer(state, dtype=np.uint64,
                                  count=25).reshape([5, 5], order='F'))
    for round_num in range(24):
        # theta_step:
        # Exclusive-or each slice-lane by state based permutation value
        array_shift = state << 1 | state >> 63
        state ^= np.bitwise_xor.reduce(state[THETA_REORDER[0], ], 1, keepdims=True) ^ np.bitwise_xor.reduce(
            array_shift[THETA_REORDER[1], ], 1, keepdims=True)
...
#res = np.unpackbits(res, axis=1)
#res = res.sum(axis=0)
print(np.unique(res))

#np.save("sha256.npy", res)
# print(np.unique(res))
...
#    res = []
#    for i in indexes:
#        res.append(np.unpackbits(my([i, second])))
#    res = np.array(res)
    #print(res.sum(axis=0))
    #print((res.sum(axis=0) % len(indexes) == 0).sum())
